...
\label{ch::navigation}
	...
	\section{Towards Rough Terrain Navigation}
		...
		\subsection{Generating a Cost-map from the Height-map}
			...
			A discrete height-map space can be converted to a cost-space representation for the purpose of foot-placement planning over rough terrain in a straight-forward way. Here, a discrete cost-map is represent by ${\cal C}\in\Re^{n\times m\times l}$. The first and second dimensions of ${\cal C}$ (of size $n$ and $m$) directly correspond to the width and depth of the discretized height-map space. The third dimension represents a discretization of the robot's yaw state, $\theta_{b,z}\in[-\pi,\pi]$ into $l$ subdivisions, and is used to represent variations of the robot's yaw with respect to $O_{hm}$ in the height-map space. We define a mapping from the height-map space to a the cost-space as follows:
				\begin{equation}
			...
			where $\Gamma$ is a static robot configuration matrix used to describe the nominal spacing between the robot's feet. $\Gamma$ is composed as follows:
				\begin{equation*}
					\Gamma \equiv \setwrap{\bar{p}_{1,e}^{hm},\bar{p}_{2,e}^{hm},\bar{p}_{3,e}^{hm},\bar{p}_{4,e}^{hm}}
					\label{eq::cost_gamma}
				\end{equation*}
			where $\bar{p}_{i,e}^{hm}=[i_{i},j_{i}]^{T}$ represent the position of each \Ith foot in the height-map space with $i_{i}\in \setwrap{1,...,m}$ and $j_{i}\in\setwrap{1,...,n}$.
			...
			To define perform the conversion between $M$ and ${\cal C}$, we first define a \emph{moving} foothold matrix $\Gamma'(p^{hm},u)$, as follows:
				\begin{equation}
					\Gamma'(p^{hm},u) = \ciel{ R_{z}^{hm}(\gamma(u))\Gamma } + p^{hm} B \\
				\end{equation}
			where  $p^{hm} = [i,j]^T$ and $u \in \setwrap{1,...,l}$ are used as a discrete representation of the robot's position and yaw configuration within the cost space, respectively; $\setwrap{\gamma(u) : Z \rightarrow \Re }$ represents a mapping from the index $u$ to a robot yaw in $O_{hm}$; $R_{z}^{hm}(\gamma(u))\in\Re^{2\times2}$ represents a rotation matrix about the $z$-axis in $O_{hm}$; and $B = [1,1,1,1]$. 

			The cost-map is then generated as follows:
				\begin{eqnarray}
					\Gamma'	 				&\equiv& \Gamma'(p^{hm},u) \nonumber \\
					{\cal H}_{v} 			&=& M(\col{\Gamma'}{v})	 \nonumber\\
					%%
					\delta {\cal H}_{v} 	&=& \nabla M(\col{\Gamma'}{v}) \nonumber\\
					%%
					{\cal C}(p^{hm},u) 		&=& k_{var} var({\cal H}) + k_{\delta}\sum_{v=1}^{4} \delta {\cal H}_{v}^{2} + k_{\theta} \gamma(u) \nonumber \\
											&\forall& p^{hm} , u , v \in {1,2,3,4}
				\end{eqnarray}
			where ${\cal H}$ is a collection heights at each foothold position; $\Nabla{\cal H}$ is a collection of corresponding gradient elements from $\Nabla M$; $k_{var}$, $k_{\delta}$ and $k_{\theta}$ are scalar weighting parameters; and $\col{*}{v}$ is extracts the $v^{th}$ column from the matrix argument $(*)$.
			...
			${\cal C}$ is used to generate an optimal, $K$-step path, which is comprised of $K$, discrete robot configurations. Moreover path is planned over rough terrain by optimizing a finite, $N$-step cost functional defined as follows:
				\begin{equation}			
					J(N) = \sum_{k=0}^N {\cal C}\wrap{p^{hm}_{k},u_{k}}
				\end{equation}	
