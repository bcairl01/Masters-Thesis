...
\label{ch::navigation}
	...
	\section{Flatland navigation and goal (feature) tracking}
		...
		\subsection{Results of LIDAR-based potential field navigation from simulation}
			...
			Figure \ref{fig::potential_field_results} shows the results of a navigation simulation in which the simulated BlueFoot robot autonmously wandered trhough an environment consisting of a room and several long corridors. A plot of the robot's trajectory shows that the robot was able to successfully navigate the region while avoiding collisions with the surrounding walls. Notably, the robot naturally avoided smaller outcove regions, which were filled with obstacles, because these regions had low relative potential, given thier relative depth with respect to the robot's position at the time they were evaluated. 
