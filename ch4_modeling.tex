...
%
			...
			In this frame, the trunk position is regarded as an offset from the origin of $O_{b}^{'}$, $p_{b,rel}^{'}$. Since there is zero rotation between $O_{0}$ and $O_{b}^{'}$, the trunk rotation vector $\theta_{b,re;}^{'}$ is directly equivalent to $\theta_{b}$ in $O_{0}$. Using these translation and rotation definitions in place of their world frame counterparts, the robot-relative joint and foot-positions, ${p}_{i,j}^{'}$ and ${p}_{i,e}^{'}$ of each \Ith leg can be computed. These values are particularly useful for planning foot and body-placements which depend directly on the location other foot positions and the relative body location, but do not necessarily depend on the position of the robot in the world coordinate frame.
