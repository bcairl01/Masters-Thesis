...
%
	...
	\section{Dynamical Model}
		...
		\subsection{Single Leg Dynamics}
			...
			The dynamics of each independent leg (with each base-frame fixed) can be derived in closed form using a Lagrangian formulated from the total potential and kinetic energy. Here, the dynamics of the kinematic chain are formulated via a minimization of energy. Masses $m_{i,j}$ are taken to be centered at each \Jth joint. These dynamics have been included as auxiliary matter in Appendix~\ref{appendix::b} using the previously defined notations.
